...
Les présentes Conditions Générales ont pour objet de définir les conditions de souscription d’un accès à la plateforme PILOT (ci-après « la Plateforme »). Elles forment le contrat conclu (ci-après « le Contrat ») entre la société HUCKLEBERRY (ci-après « la Société ») et toute personne physique ou morale (ci-après « le Client ») souscrivant un accès à la Plateforme.
...
Le Client s’interdit de souscrire, sans autorisation préalable express de la Société, à plus d’un accès à la fois à la Plateforme.
...
Le présent Contrat est conclu pour une durée indeterminée courant à compter de la confirmation, par la Société, de la souscription de l’accès à la Plateforme. Il pourra être résilié selon les modalités prévues à l’article RESILIATION du présent contrat. 
...
## ARTICLE 5: LES OBLIGATIONS DE LA SOCIETE

### ARTICLE 5.1 – DISPONIBILITE
...
### ARTICLE 5.2 – LA CONFIDENTIALITE ET LA SECURITE DES ACCES
...
Amazon Web Services EC2 (application) : [http://aws.amazon.com/fr/ec2/sla/?nc2=h_ls](http://aws.amazon.com/fr/ec2/sla/?nc2=h_ls)

Amazon Web Services S3 (Hébergement de fichiers) : [http://aws.amazon.com/fr/s3/sla/?nc2=h_ls](http://aws.amazon.com/fr/s3/sla/?nc2=h_ls)

Heroku SLA : [https://www.heroku.com/policy/promise](https://www.heroku.com/policy/promise)
...
ARTICLE 6.6 – REVERSIBILITE

En cas de résiliation du Contrat, la Société assure au Client la restitution de ses données, par téléchargement depuis un serveur de fichier ou dépôt sur un serveur de fichier du Client.

Le Contenu sera livré dans un dossier contenant : 

* Un fichier texte (format TXT) contenant sa dernière version courante ;

* Un dossier contenant toutes les versions du Contenu (format TXT) ;

* Un fichier contenant tous les commentaires associés au Contenu (format TXT) ;

* Un dossier contenant tous les fichiers liés (tous formats nécessaires).

Chaque campagne sera livrée dans un dossier contenant

* Un fichier (format TXT) contenant tous les informations la concernant (Titre, Dates début/fin, description, etc..) ;

* Un dossier contenant tous les fichiers qui lui sont liés (aux formats des fichiers) ;

* Un dossier contenant les Contenus attachés à la campagne (même structure que celle citée ci-dessus).

Tous les fichiers de la médiathèque pourront être transférés vers un serveur de fichier du Client ou, si leur volume le permet, être téléchargés depuis un serveur de fichier de la Société.

En principe, la prestation de réversibilité est à la charge de la Société si le format d’export tel que décrit ci dessus est accepté par le Client.
...
### ARTICLE 6.1 – CONFORMITE AUX BESOINS
...
## ARTICLE 16: INTEGRALITE ET INDIVISIBILITE
...
Le Client se réserve la faculté de transférer le Contrat à toute société le contrôlant, qu’il contrôle ou sous contrôle commun, directement ou indirectement, avec lui (au sens donné au terme "contrôle" par l’article L.233-3 du Code de commerce).
